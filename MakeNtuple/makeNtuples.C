...
double computeInvariantMass_reco(ParticleFlowCandidate *dau1,
                                 ParticleFlowCandidate *dau2) {

  TLorentzVector p4_1;
  p4_1.SetPtEtaPhiM(dau1->PT, dau1->Eta, dau1->Phi, dau1->Mass);
  TLorentzVector p4_2;
  p4_2.SetPtEtaPhiM(dau2->PT, dau2->Eta, dau2->Phi, dau2->Mass);
  TLorentzVector totalP4 = p4_1 + p4_2;
  return totalP4.M(); // Returns the invariant mass
}

double computeInvariantMass(GenParticle *dau1, GenParticle *dau2) {
...
//   // PDG IDs for charm hadrons: D-mesons, Lambda_c, etc.
//   std::vector<int> charmHadrons = {411, -411, 421,  -421,
//                                    431, -431, 4122, -4122};
//   return std::find(charmHadrons.begin(), charmHadrons.end(), abs(pdgId)) !=
//          charmHadrons.end();
// }

bool isD0_Meson(int pdgId) {
  std::vector<int> dMesons = {421, -421}; // D0, D̅0
  return std::find(dMesons.begin(), dMesons.end(), abs(pdgId)) != dMesons.end();
}

bool isDp_Meson(int pdgId) {
  std::vector<int> dMesons = {411, -411}; // D+, D-
  return std::find(dMesons.begin(), dMesons.end(), abs(pdgId)) != dMesons.end();
}

bool isDs_Meson(int pdgId) {
  std::vector<int> dMesons = {431, -431}; // Ds+, Ds-
  return std::find(dMesons.begin(), dMesons.end(), abs(pdgId)) != dMesons.end();
}

bool isCB_Meson(int pdgId) {
  int temp = int(abs(pdgId)) / 1000;
  if (temp == 4)
    return true; // Charm baryons (e.g., Lambda_c, Sigma_c, etc.)
  else
    return false;
}

bool isD_StarMeson(int pdgId) {
  std::vector<int> dStarMesons = {413, -413}; // D*+, D*-, D*0, D̅*0
  // std::vector<int> dStarMesons = {413,-413}; // D*+, D*-, D*0, D̅*0
  return std::find(dStarMesons.begin(), dStarMesons.end(), abs(pdgId)) !=
         dStarMesons.end();
}
...
MesonType traceAncestry(ParticleFlowCandidate *cand,
                        TClonesArray *branchParticle) {
  if (!cand || !branchParticle)
    return {false, false, false, false, false}; // Safety check

  // assumption - take only one mother particle
  bool foundD0 = false;
  bool foundDp = false;
  bool foundDs = false;
  bool foundCB = false;
  bool foundDStar = false;
  for (Int_t k = 0; k < cand->Particles.GetEntriesFast(); ++k) {
    if (k > 0)
      continue;
    // link the canidate Pflow with its corresponding truth particle
    const TObject *object1 = cand->Particles.At(k);
    if (!object1)
      continue;

    GenParticle *truthParticle = (GenParticle *)object1;
    int motherIndex = truthParticle->M1; // Get mother index

    while (motherIndex >= 0 && motherIndex < branchParticle->GetEntriesFast()) {
      GenParticle *motherParticle =
          (GenParticle *)branchParticle->At(motherIndex);
      GenParticle *D1Particle =
          (GenParticle *)branchParticle->At(motherParticle->D1);
      GenParticle *D2Particle =
          (GenParticle *)branchParticle->At(motherParticle->D2);
      if (!motherParticle)
        break;

      int motherPdgId = motherParticle->PID; // PDG ID of mother particle
      // std::cout << "Checking Mother PDG ID: " << motherPdgId << std::endl;

      if (isD0_Meson(motherPdgId)) {
        // double massreco = computeInvariantMass(D1Particle,D2Particle);
        foundD0 = true;
        break; // Stop if we found a D0
      } else if (isDp_Meson(motherPdgId)) {
        foundDp = true;
        break; // Stop if we found a D+ meson
      } else if (isDs_Meson(motherPdgId)) {
        foundDs = true;
        break; // Stop if we found a Ds meson
      } else if (isCB_Meson(motherPdgId)) {
        foundCB = true;
        break; // Stop if we found a CB meson
      } else if (isD_StarMeson(motherPdgId)) {
        //                std::cout << "Found D*-meson with PDG ID: " <<
        //                motherPdgId<< " and mass =  " << motherParticle->Mass
        //                <<std::endl;
        foundDStar = true;
        break; // Stop if we found a D*-meson
      }

      motherIndex = motherParticle->M1; // Move to the next mother in ancestry
    }
  }

  MesonType mesonType{foundD0, foundDp, foundDs, foundCB, foundDStar};

  return mesonType; // Return the meson type
}
...
template <class T1, class T2> double deltaR(const T1 &a, const T2 &b) {
...
public:
  ...
  std::pair<FatJetLabel, const GenParticle *>
  getLabel(const Jet *jet, const TClonesArray *branchParticle) {
    genParticles_.clear();
    ...
    if (genParticles_.size() != processed_.size())
      throw std::logic_error(
          "[FatJetMatching] Not all genParticles are processed!");
...
private:
  std::pair<FatJetLabel, const GenParticle *>
  top_label(const Jet *jet, const GenParticle *parton) {
    // top
    ...
    if (!w_from_top || !b_from_top)
      throw std::logic_error(
          "[FatJetMatching::top_label] Cannot find b or W from top decay!");
    ...
    if (isHadronic(w_from_top)) {
      ...
      if (debug_) {
        using namespace std;
        cout << "jet: (" << jet->PT << ", " << jet->Eta << ", " << jet->Phi
             << ", " << jet->P4().Energy() << ")" << endl;
        cout << "top: ";
      ...
      if (wdaus.size() < 2)
        throw std::logic_error(
            "[FatJetMatching::top_label] W decay has less than 2 quarks!");
      //    if (wdaus.size() >= 2)
      {
      ...
      if (!lep)
        throw std::logic_error("[FatJetMatching::top_label] Cannot find "
                               "charged lepton from leptonic W decay!");
  ...
  std::pair<FatJetLabel, const GenParticle *>
  c_label(const Jet *jet, const GenParticle *parton) {
    auto cquark = getFinal(parton);
    double dr_cj = deltaR(jet, cquark);
    auto pdgid_c = std::abs(cquark->PID);
    if (dr_cj < jetR_) {
      if (pdgid_c == ParticleID::p_c) {
        return std::make_pair(FatJetLabel::QCD_c, cquark);
      }
    }
    return std::make_pair(FatJetLabel::Invalid, nullptr);
  }
  ...
  std::pair<FatJetLabel, const GenParticle *>
  w_label(const Jet *jet, const GenParticle *parton) {
    ...
    if (isHadronic(w)) {
      ...
      if (wdaus.size() < 2)
        throw std::logic_error(
            "[FatJetMatching::w_label] W decay has less than 2 quarks!");
      //    if (wdaus.size() >= 2)
      {
        double dr_q1 = deltaR(jet, wdaus.at(0));
        double dr_q2 = deltaR(jet, wdaus.at(1));
  ...
  std::pair<FatJetLabel, const GenParticle *>
  z_label(const Jet *jet, const GenParticle *parton) {
    ...
    if (isHadronic(z)) {
      ...
      if (zdaus.size() < 2)
        throw std::logic_error(
            "[FatJetMatching::z_label] Z decay has less than 2 quarks!");
      //    if (zdaus.size() >= 2)
      {
  ...
  std::pair<FatJetLabel, const GenParticle *>
  higgs_label(const Jet *jet, const GenParticle *parton) {
    ...
    if (is_hvv) {
      ...
      if (debug_) {
        using namespace std;
        cout << "Found " << hvv_quarks.size() << " quarks from Higgs decay"
             << endl;
        ...
        cout << "Found " << hvv_leptons.size() << " leptons from Higgs decay"
             << endl;
      ...
      if (hdaus.size() < 2)
        throw std::logic_error("[FatJetMatching::higgs_label] Higgs decay has "
                               "less than 2 quarks!");
      //    if (zdaus.size() >= 2)
      {
        ...
        if (dr_q1 < jetR_ && dr_q2 < jetR_) {
          ...
          } else if (pdgid_q1 == ParticleID::p_g &&
                     pdgid_q2 == ParticleID::p_g) {
...
private:
  void printGenInfoHeader() const {
    ...
    cout << right << setw(6) << "#"
         ...
         << "Chg"
         << "  " << setw(10) << "Mass"
         << "  " << setw(48) << " Momentum" << left << "  " << setw(10)
         << "Mothers"
         << " " << setw(30) << "Daughters" << endl;
  }

  void printGenParticleInfo(const GenParticle *genParticle,
                            const int idx) const {
    using namespace std;
    ...
    cout << right << "  " << setw(3) << genParticle->Charge << "  "
         << TString::Format("%10.3g",
                            genParticle->Mass < 1e-5 ? 0 : genParticle->Mass);
    cout << left << setw(50)
         << TString::Format("  (E=%6.4g pT=%6.4g eta=%7.3g phi=%7.3g)",
                            genParticle->P4().Energy(), genParticle->PT,
                            genParticle->Eta, genParticle->Phi);
  ...
  bool isHadronic(const GenParticle *particle, bool allow_gluon = false) const {
    ...
    if (!particle)
      throw std::invalid_argument(
          "[FatJetMatching::isHadronic()] Null particle!");
  ...
  std::vector<const GenParticle *>
  getDaughters(const GenParticle *particle) const {
  ...
  std::vector<const GenParticle *>
  getDaughterQuarks(const GenParticle *particle, bool allow_gluon = false) {
    std::vector<const GenParticle *> daughters;
    for (int idau = particle->D1; idau <= particle->D2; ++idau) {
      const auto *dau = genParticles_.at(idau);
      auto pdgid = std::abs(dau->PID);
...
void makeNtuples(TString inputFile, TString outputFile,
  ...
  arrayVars["part_isFromD0"];
  arrayVars["part_isFromDp"];
  arrayVars["part_isFromDs"];
  arrayVars["part_isFromCB"];
  arrayVars["part_isFromDStar"];
  arrayVars["part_massReco"];
  ...
  FatJetMatching fjmatch(0.4);

  // Loop over all events
  int num_processed = 0;
  for (Long64_t entry = 0; entry < allEntries; ++entry) {
    if (entry % 1000 == 0) {
      std::cerr << "processing " << entry << " of " << allEntries << " events."
                << std::endl;
    }
    ...
    for (Int_t i = 0; i < branchJet->GetEntriesFast(); ++i) {
      ...
      if (fjmatch.event_type() == FatJetMatching::EventType::Top) {
        ...
        floatVars["is_signal"] = (label.first == FatJetMatching::Top_bcq ||
                                  label.first == FatJetMatching::Top_bqq ||
                                  label.first == FatJetMatching::Top_ben ||
                                  label.first == FatJetMatching::Top_bmn);
      } else if (fjmatch.event_type() == FatJetMatching::EventType::Higgs) {
        floatVars["is_signal"] = (label.first > FatJetMatching::H_all &&
                                  label.first < FatJetMatching::QCD_all);
      } else if (fjmatch.event_type() == FatJetMatching::EventType::W) {
        floatVars["is_signal"] = (label.first > FatJetMatching::W_all &&
                                  label.first < FatJetMatching::Z_all);
      } else if (fjmatch.event_type() == FatJetMatching::EventType::Z) {
        floatVars["is_signal"] = (label.first > FatJetMatching::Z_all &&
                                  label.first < FatJetMatching::H_all);
      }

      if (fjmatch.event_type() != FatJetMatching::EventType::QCD &&
          floatVars["is_signal"] == 0) {
      ...
      std::vector<int> foundD0;
      std::vector<int> foundDp;
      std::vector<int> foundDs;
      std::vector<int> foundCB;
      std::vector<int> foundDStar;
      std::vector<float> massReco_v;
      int D0decay = 0;
      ParticleFlowCandidate *candD1 = nullptr;
      ParticleFlowCandidate *candD2 = nullptr;
      double mass_Reco = 0;
      for (Int_t j = 0; j < jet->Constituents.GetEntriesFast(); ++j) {
        ...
        if (object->IsA() == GenParticle::Class()) {
          ...
          foundDs.push_back(0);
          foundCB.push_back(0);
          foundDStar.push_back(0);
        } else if (object->IsA() == ParticleFlowCandidate::Class()) {
          particles.emplace_back((ParticleFlowCandidate *)object);

          ParticleFlowCandidate *cand = nullptr;
          cand = (ParticleFlowCandidate *)object;
          auto result = traceAncestry(cand, branchParticle);
          foundD0.push_back(result.isD0);
          foundDp.push_back(result.isDp);
          foundDs.push_back(result.isDs);
          foundCB.push_back(result.isCB);
          foundDStar.push_back(result.isDStar);
          ...
          if (result.isD0) {
            ...
            if (D0decay == 2) {
              candD2 = cand;
              mass_Reco = computeInvariantMass_reco(candD1, candD2);
              //        std::cout << "mass_Reco =  " << mass_Reco << std::endl;
              //        std::cout << " PT1=  " << candD1->PT<< " PT2=  " <<
              //        candD2->PT<< std::endl;
            }
          }
          massReco_v.emplace_back(mass_Reco);
        }
      ...
      // std::sort(particles.begin(), particles.end(), [](const auto &a, const
      // auto &b) { return a.pt > b.pt; });
      ...
      for (const auto &p : particles) {
        ...
        arrayVars["part_eta"].push_back(p.eta);
        arrayVars["part_phi"].push_back(p.phi);
        arrayVars["part_mass"].push_back(p.mass);
        arrayVars["part_deta"].push_back((jet->Eta > 0 ? 1 : -1) *
                                         (p.eta - jet->Eta));
        ...
        arrayVars["part_isFromD0"].push_back(foundD0[k]);
        arrayVars["part_isFromDp"].push_back(foundDp[k]);
        arrayVars["part_isFromDs"].push_back(foundDs[k]);
        arrayVars["part_isFromCB"].push_back(foundCB[k]);
        arrayVars["part_isFromDStar"].push_back(foundDStar[k]);
        arrayVars["part_massReco"].push_back(massReco_v[k]);
      }
  ...
  tree->Write();
  std::cerr << TString::Format("** Written %d jets to output %s", num_processed,
                               outputFile.Data())
            << std::endl;
