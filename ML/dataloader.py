...
class ParticleJetDataset(Dataset):
    ...
    def __normalize_full_data__(self):
        for key in self.particle_variables + self.jet_variables:
            ...
            if isinstance(self.full_data_array[key], np.ndarray):
                ...
                ) / (np.std(self.full_data_array[key]) + 1e-12)
    ...
    def __getitem__(self, idx):
        ...
        labels_particle = np.full_like(
            isFromD, 0
        )  # Default to "Anything Else" (Class 0)
        labels_particle[isFromD > 0] = 1  # Class 1: D meson
        labels_particle[isFromDStar > 0] = (
            2  # Class 2: D* meson (overwrites D if both exist)
        )
